...
class Configurator(ConfiguratorBase):
    def __init__(self, host='localhost', port=4040):
        ...
        *host:* hostname of the computer where the AMDA board is attached
        ...
        self.host = host
        self.port = port
        self.client = caer_communication.caerCommunicationControlServer(host=host, port_control=port)
        self.open()

    def load_parameter_definitions(self, filename):
        '''
        ...
        alld = dict()
        for biases_root in doc.findall('.//chip/node[@name="bias"]'):
